...
inline Service::Service(): srpc::RPCService("SignUp")
{
...
{
	auto *task = this->create_rpc_client_task("/SignUp/Echo", std::move(done));
...
inline WFFuture<std::pair<EchoResponse, srpc::RPCSyncContext>> TRPCClient::async_Echo(const EchoRequest *req)
{
	...
	auto *pr = new WFPromise<RESULT>();
	auto fr = pr->get_future();
	auto *task = this->create_rpc_client_task<EchoResponse>("/SignUp/Echo", srpc::RPCAsyncFutureCallback<EchoResponse>);
