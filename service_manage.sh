...
view_logs() {
    case "$2" in
        could|couldserver)
            echo "Check the main server log:"
            echo "================="
            tail -f $LOG_DIR/couldserver.log
            ;;
        signup)
            echo "Check the registration server log:"
            echo "=================="
            tail -f $LOG_DIR/signup.log
            ;;
        rabbit)
            echo "Check the log file of the Rabbit program:"
            echo "=================="
            tail -f $LOG_DIR/rabbit.log
            ;;
        signin)
            echo "Check the log of the sign-in program:"
            echo "=================="
            tail -f $LOG_DIR/signin.log
            ;;
        *)
            echo "method of application: $0 log {could|signup|rabbit}"
            echo "  Optional log entries:"
            echo "    could    - Main server log"
            echo "    signup   - Registration server log"
            echo "    rabbit   - Rabbit Program Log"
            echo "    signin   - Signin program log"
            exit 1
            ;;
    esac
}
